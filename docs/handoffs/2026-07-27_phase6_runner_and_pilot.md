...
- `python -m pytest -q`：`64 passed in 30.55s`。
...
- 排他锁针对同一台机器的本地文件系统；不支持多主机网络文件系统协调。
