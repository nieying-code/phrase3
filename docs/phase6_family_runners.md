...
这些执行器的科学职责与 E3 分离。纯family模型变更不会修改E3组件；但
两条执行链共同依赖的复现基础设施（例如换行策略、环境验证和原子写入）会
明确进入两类组件指纹，因此此类公共变更会同时使旧门槛失效，不能通过
人为迁移继续使用。
...
实际环境哈希还包含严格的CPython 3.12.10和稳定硬件身份。执行前要求
tracked工作树干净，未跟踪文件只能位于`outputs/`。受控源码、YAML和锁
文件必须是LF字节；新实验周期必须使用新的输出根目录。

family制品按`result → finalized manifest（含result SHA-256）→ registry
→ projection`顺序写入。投影会重新交叉核验CSV关键字段、manifest和结果
文件哈希，未最终化或发生字节变化的运行不能进入门槛。
