...
def test_repro_v3_p2_pilots_and_complete_compute_gate() -> None:
    ...
    assert audit["audit_schema"] == (
        "phase6_repro_v3_p2_e3_pilots_and_compute_gate_v1"
    )
    source = audit["source"]
    ...
    hours = family["projected_wall_hours"]
    assert set(hours) == {"E1", "E2", "E3", "E4", "E5"}
    assert all(value > 0.0 for value in hours.values())

    gate = audit["compute_gate"]
    total_hours = sum(hours.values())
    largest_hours = max(hours.values())
    assert math.isclose(
    ...
    expected_gate = total_hours <= 168.0 and largest_hours <= 72.0
    assert gate["projection_status"] == "passed"
    assert gate["compute_gate_passed"] is expected_gate
    assert gate["formal_execution_authorized"] is expected_gate
