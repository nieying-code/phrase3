"""Bounded status reader for M1 development runs; never parses large results."""
...
def read_status(output_root: Path, run_id: str | None) -> dict[str, Any]:
    ...
    if path.stat().st_size > MAX_STATUS_BYTES:
        raise RuntimeError(f"bounded M1 status file exceeds 16 KiB: {path}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--output-root", default="outputs/phase6_m1_procurement_cap_v1")
