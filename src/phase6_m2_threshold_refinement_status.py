"""Bounded status reader; never loads large refinement results."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from .phase6_m2_threshold_refinement import OUTPUT_ROOT, _run_directory

MAX_BYTES = 16 * 1024
def read_status(output_root: Path, run_id: str | None):
    path = output_root / "development/threshold_refinement_projection.json" if run_id is None else _run_directory(output_root, run_id) / "status_summary.json"
